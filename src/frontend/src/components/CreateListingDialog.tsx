...
import { ProductCondition, Region, SwissCanton, Country } from '../backend';
...
export default function CreateListingDialog({ open, onOpenChange }: CreateListingDialogProps) {
  ...
  const [region, setRegion] = useState<string>('geneva');
  ...
  const convertToRegion = (regionValue: string): Region => {
    // For now, only Swiss cantons are supported for creating listings
    return {
      __kind__: 'swissCanton',
      swissCanton: regionValue as SwissCanton
    };
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await checkComplianceAndExecute('createListing', executeCreateListing);
    } catch (error: any) {
