...
import { useIsFavorite, useToggleFavorite, useSendMessage } from '../hooks/useQueries';
...
export default function ListingDetailDialog({ 
  ...
  const handleContactSeller = async () => {
    if (!isAuthenticated) {
      toast.error('Connecte-toi pour contacter la vendeuse');
      return;
    }

    const executeContactSeller = async () => {
      toast.error('Impossible de contacter la vendeuse : les informations du vendeur ne sont pas disponibles');
    };

    try {
      await checkComplianceAndExecute('sendMessage', executeContactSeller);
    } catch (error: any) {
